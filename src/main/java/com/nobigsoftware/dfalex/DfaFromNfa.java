...
class DfaFromNfa<RESULT>
{
	...
	private final DfaAmbiguityResolver<RESULT> m_ambiguityResolver;
	...
	private final LinkedList<Integer> m_tempNfaClosureList = new LinkedList<>();
	...
	public DfaFromNfa(Nfa<RESULT> nfa, int[] nfaStartStates, DfaAmbiguityResolver<RESULT> ambiguityResolver)
	{
		m_nfa = nfa;
	...
	private void _build()
	{
		...
		for (int stateNum = 0; stateNum < m_dfaStateSignatures.size(); ++stateNum)
		{
			...
			DfaStateSignatureCodec.expand(dfaStateSig, new IntToVoid() {
				@Override
				public void accept(int state) {
					m_nfa.forStateTransitions(state, transitionQ);
				}
			});
			
			//sort all the transitions by first character
			Collections.sort(transitionQ, new Comparator<NfaTransition>() {
				@Override
				public int compare(NfaTransition arg0, NfaTransition arg1) {
	...
	private void _addNfaStateAndEpsilonsToSubset(CompactIntSubset dest, int stateNum)
	{
		...
		{
			m_nfa.forStateEpsilons(newNfaState, dest, m_tempNfaClosureList);
		}
	}
    ...
    private DfaStateInfo _createStateInfo(IntListKey sig, List<NfaTransition> transitions)
	{
		...
		DfaStateSignatureCodec.expand(sig,
				new IntToVoid() {
					@Override
					public void accept(int nfastate) {
