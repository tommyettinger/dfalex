...
class DfaStateSignatureCodec
{
	...
	public static void expand(IntListKey key, final IntToVoid target)
	{
		key.forData(new ObjIntToVoid<int[]>() {
            @Override
            public void accept(int[] buf, int len) {
                expand(buf, len, target);
            }
        });
	}
	
	public static void expand(int[] sigbuf, final int siglen, IntToVoid target)
	{
		if (siglen <= 0)
		{
			return;
