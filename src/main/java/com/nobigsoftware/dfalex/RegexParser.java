...
class RegexParser {
    ...
    public static CharRange WORD_CHARS = CharRange.builder().addRange('a','z').addRange('A', 'Z').addRange('0', '9').addChars("_").build();
    ...
    private Matchable _parse()
    {
        ...
        for (;;)
        {
            ...
            while(m_stateStack.size()-1 < m_symStack.length())
            {
                ...
                if (st == null)
                {
                    throw new RuntimeException("Internal bug encountered parsing regular expression: "+m_src);
                }
                m_stateStack.add(st);
            }
        ...
        {
            throw new IllegalArgumentException("Invalid regular expression: \""+m_src+"\" has error at position "+maxpos);
        }
    ...
    private char _parseCharEscape()
    {
        ...
        try
        {
            ...
            switch(c)
            {
                ...
                case 'x':
                case 'u':
                return(char)Integer.parseInt(m_src.substring(spos+1, m_readPos), 16);
    ...
    private static DfaState<Action> _buildParserDfa() //produces A
    {
        ...
        //S: C | S '|' C
        bld.addPattern(Spos.then("C"), new Action() { public void apply(RegexParser x) {    x._push("S", x._pop(1));    }});
        bld.addPattern(Spos.then("S:|"), new Action() { public void apply(RegexParser x) {    x._push("|", null);    }});
        bld.addPattern(Spos.then("S|C"), new Action() { public void apply(RegexParser x) {    
        ...
        //C: e | C A
        bld.addPattern(Cpos, new Action() { public void apply(RegexParser x) {    x._push("C", Pattern.EMPTY);    }});
        bld.addPattern(Cpos.then("CA"),  new Action() { public void apply(RegexParser x) {    
            Matchable p2 = x._pop(1);
        ...
        //A: A? | A+ | A*
        bld.addPattern(Apos.then("A:?"), new Action() { public void apply(RegexParser x) {    x._push("A", Pattern.maybe(x._pop(1)));   }});
        bld.addPattern(Apos.then("A:+"), new Action() { public void apply(RegexParser x) {    x._push("A", Pattern.repeat(x._pop(1)));    }});
        bld.addPattern(Apos.then("A:*"), new Action() { public void apply(RegexParser x) {    x._push("A", Pattern.maybeRepeat(x._pop(1)));    }});
        
        //A: GROUP
        bld.addPattern(Apos.then(":("), new Action() { public void apply(RegexParser x) {    x._push("(",null);    }});
        bld.addPattern(Apos.then("(S:)"), new Action() { public void apply(RegexParser x) {    x._push("A",x._pop(2));    }});
        
        //A: literal | .
        bld.addPattern(Apos.then(":").then(CharRange.builder().addChars(".()[]+*?|\\").invert().build()), new Action() { public void apply(RegexParser x) {
            CharRange range;
            char c = x._lastChar();
            if (!x.m_caseI)
            {
        ...
        bld.addPattern(Apos.then(":."), new Action() { public void apply(RegexParser x) {    x._push("A", CharRange.ALL);    }});
        ...
        final Pattern charEscape = Pattern.match(":\\").then(Pattern.anyOf(
                ...
                Pattern.match("u").then(CharRange.HEXDIGITS).then(CharRange.HEXDIGITS).then(CharRange.HEXDIGITS).then(CharRange.HEXDIGITS),
                Pattern.match("c").then(CharRange.builder().addRange('a', 'z').addRange('A','Z').build()),
                CharRange.builder().addChars("xucdDwWsS").invert().build()));
        final Pattern classEscape = Pattern.match(":\\").then(Pattern.anyCharIn("dDsSwW"));
        
        bld.addPattern(Apos.then(charEscape), new Action() { public void apply(RegexParser x) {    x._push("A",CharRange.single(x._parseCharEscape()));    }});
        bld.addPattern(Apos.then(classEscape), new Action() { public void apply(RegexParser x) {    x._push("A",x._parseClassEscape());    }});
        
        //A: [R] | [^R]
        bld.addPattern(Apos.then(":[^"), new Action() { public void apply(RegexParser x) {    
        ...
        bld.addPattern(Apos.then("[R:]"), new Action() { public void apply(RegexParser x) {    
            x._pop(2);
            if (x.m_caseI)
            {
        ...
        bld.addPattern(Rpos, new Action() { public void apply(RegexParser x) {    x._push("R", null);    }});
        bld.addPattern(Rpos.then("R").then(classEscape), new Action() { public void apply(RegexParser x) {    
            x.m_charBuilder.addRange(x._parseClassEscape());
            x._pop(0);
            }});
        bld.addPattern(Rpos.then("Rc"), new Action() { public void apply(RegexParser x) {    
            x._pop(1);
            x.m_charBuilder.addRange(x.m_clast, x.m_clast);
            }});
        bld.addPattern(Rpos.then("Rc:-"), new Action() { public void apply(RegexParser x) {    x._push("-", null);    }});
        bld.addPattern(Rpos.then("Rc-c"), new Action() { public void apply(RegexParser x) {    
            x._pop(3);
        ...
        //class chars
        bld.addPattern(cpos.then(":").then(CharRange.builder().addChars("-[]\\").invert().build()), new Action() { public void apply(RegexParser x) {
            x.m_cprev = x.m_clast;
            x.m_clast = x._lastChar();
            x._push("c", null);
            }});
        bld.addPattern(cpos.then(charEscape), new Action() { public void apply(RegexParser x) {
            x.m_cprev = x.m_clast;
