...
class DfaMinimizer<RESULT> {
    ...
    private int[][] _createBackReferences() {
        ...
        final int[] backRefCounts = new int[m_origStates.size()];
        for (int st = 0; st < nstates; ++st) {
            m_origStates.get(st).forEachTransition(new ObjToVoid<NfaTransition>() {
                public void accept(NfaTransition trans) {
                    backRefCounts[trans.m_stateNum]++;
                }
            });
        }
        final int[][] backrefs = new int[nstates][];
        for (int st = 0; st < nstates; ++st) {
        ...
        for (int st = 0; st < nstates; ++st) {
            captureFix[0] = st;
            m_origStates.get(st).forEachTransition(
                    new ObjToVoid<NfaTransition>() {
                        public void accept(NfaTransition trans) {
