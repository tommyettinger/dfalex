...
public class Nfa<MATCHRESULT>
{
    ...
    public Iterable<NfaTransition> getStateTransitions(int state)
    {
        ...
        return (list != null ? list : Collections.<NfaTransition>emptyList());
    }
    ...
    public int Disemptify(final int state)
    {
        ...
        //first find all epsilon-reachable states
        {
            ...
            {
				List<Integer> list = m_stateEpsilons.get(reachable.get(i));
				if (list != null) {
	...
	void forStateEpsilons(int state, CompactIntSubset dest, Collection<Integer> alternate)
	{
		List<Integer> list = m_stateEpsilons.get(state);
		if (list != null) {
			for (Integer i : list)
	...
	void forStateTransitions(int state, Collection<NfaTransition> dest)
	{
		List<NfaTransition> list = m_stateTransitions.get(state);
		if (list != null)
		{
			for(NfaTransition e : list)
				dest.add(e);
		}
	}
}
